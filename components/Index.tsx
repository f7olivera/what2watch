import { Box, Center, Flex, Heading, Icon, Select } from "@chakra-ui/react";
...
function Index({ media = "movie", id }: Props) {
  ...
  const [moviesQuery, dispatch] = React.useReducer(
    moviesReducer,
    initialState,
    init
  );
  ...
  const { data, isLoading, isFetched } = useQuery<
    ...
    unknown,
    IMovieResults
  >([...moviesQuery.queryKey, page], moviesQuery.queryFunction);

  React.useEffect(() => {
    if (window.location.pathname === "/") return;
    document.title = `${media === "movie" ? "Movies" : "TV Shows"} - ${
      /top|popular/i.test(moviesQuery.category)
        ? moviesQuery.category.split(" ")[0]
        : moviesQuery.category
    }`;
  }, [media, moviesQuery]);
  ...
  return (
    <>
      ...
      >
        ...
        />
        <Center display="flex" flexDirection="column" gap="4rem" zIndex="2">
          <Box px="5%">
            <Heading size="2xl" mb="0.75rem" zIndex="2" color="white">
            ...
            >
              ...
              {media === "tv" ? (
                <option value={`/tv/airing`}>Airing now</option>
              ) : (
                ""
              )}
              <optgroup label="Genres">
                {genres2.map((genre) => (
                  <option value={`/${media}/genre/${genre.id}`} key={genre.id}>
                    {genre.name}
                  </option>
                ))}
              </optgroup>
