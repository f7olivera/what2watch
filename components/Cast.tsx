...
function Cast({ media, movie_id }: Props) {
  const { data: cast } = useQuery<IActor[], unknown, IActor[]>(
    ["cast", media, movie_id],
    fetchCast
  );

  const scrollCast = (event: PointerEvent<HTMLDivElement>) => {
    const castContainer: HTMLDivElement | null =
  ...
  return (
    ...
    >
      {cast &&
        cast.slice(0, Math.min(cast.length, 10)).map((actor) => (
          <Flex
            margin="0.5rem"
            zIndex={1}
            key={actor.id}
