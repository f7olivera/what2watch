...
function Search() {
  ...
  const {
    data,
    isLoading
  } = useQuery<IMovie[] | IMovieResults, unknown, IMovieResults>(['search', q, page], fetchSearch);
  ...
  return isLoading || !data ? (
    ...
    <div className='main-wrapper'>
      <Movies list={data.results || []} isLoading={false} category={`Results for "${q}"`} media=''/>
      <Paginator url={typeof window !== 'undefined' ? window.location.pathname : '/'} page={page} setPage={setPage}
                 totalPages={totalPages || 1}/>
    </div>
  ) : (
    <Flex className='main-wrapper' flexDirection='column'
          alignItems='center' justifyContent='center' height='83vh'>
