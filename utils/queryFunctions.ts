...
export const fetchDiscover = async ({
  ...
  const url = `${BASE_URL}/${
    ["movie", "tv"].includes(`${category}`) ? category : "movie"
  }/discover${id ? `/${id}` : ""}?page=${page}`;
  const response = await fetchResource(url);
...
export const fetchSearch = async ({
  ...
  const results: IMovie[] = rawJson.results.filter(
    (elem: IMovie) => elem.media_type === "movie" || elem.media_type === "tv"
  );
  return { ...rawJson, results };
};

export const fetchAiring = async ({
  queryKey,
}: QueryFunctionContext<QueryKey>) => {
  const [_, page] = queryKey;
...
export const fetchTrailers = async ({
  ...
  const response = await fetchResource(url);
  return await response.results.filter(
...
export const fetchCertification = async ({
  ...
  const response = await fetchResource(url);
  const certifications = await response.results.filter(
    (elem: {
      iso_3166_1: string;
      release_dates: { certification: string }[];
    }) => elem.iso_3166_1 === "US"
  );
  return certifications?.length && certifications[0].release_dates.length
